import { getFirestore, doc, getDoc, getDocs, collection, query, where, writeBatch, deleteDoc, deleteField, updateDoc, setDoc, serverTimestamp, onSnapshot } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
export function showApp() {
    ...
    document.getElementById('assign-delivery-section').style.display = 'none';
    
    if (currentUser.role === 'driver' || currentUser.role === 'warehouse_supervisor') {
        ...
        // Show delivery assignment form for admin or users with permission
        if(currentUser.role === 'admin' || currentUser.can_create_pod) {
            document.getElementById('assign-delivery-section').style.display = 'block';
        }
...
export async function handleCompleteDelivery(e) {
    ...
    if (completionSignaturePad.isEmpty()) {
     ...
     if (!capturedPhotoDataUrl) {
        showNotification("Please take a photo of the delivery.", true);
        return;
    }
...
export async function handlePhotoUpload(event) {
    ...
    showLoader();
    try {
        // Resize to max 800x800, 70% quality to reduce base64 string size
        const resizedDataUrl = await resizeImage(file, 800, 800, 0.7);
        setCapturedPhotoDataUrl(resizedDataUrl);
        ...
        showNotification("Could not process photo. Please try again.", true);
    } finally {
        hideLoader();
    }
}
...
export function generateQRCodes(container = document) {
     ...
     if (!data) return;
     const publicUrl = window.location.origin + window.location.pathname.replace('index.html', '') + '?podId=' + data.deliveryId;
...
export async function openAdminPanel() {
    ...
    try {
        ...
        const permissionListEl = document.getElementById('pod-permission-list');
        userListEl.innerHTML = '';
        permissionListEl.innerHTML = '';
        
        if(users.length === 0) {
            userListEl.innerHTML = `<p class="text-gray-500 text-center p-4">No users found.</p>`;
        } else {
            users.forEach(user => {
                ...
                userEl.innerHTML = `
                    <div class="flex justify-between items-center md:block col-span-2">
                        <div>
                ...
                // Add to permission list if they are not a driver
                if(user.role !== 'driver') {
                    const permissionEl = document.createElement('div');
                    permissionEl.className = "p-3 border-b flex justify-between items-center";
                    permissionEl.innerHTML = `
                        ...
                        <div class="flex items-center">
                            <label for="perm-${user.id}" class="mr-2 text-sm">Can Create POD:</label>
                            <input type="checkbox" id="perm-${user.id}" data-uid="${user.id}" class="h-5 w-5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 pod-permission-checkbox" ${user.can_create_pod ? 'checked' : ''}>
                        </div>
                    `;
                    permissionListEl.appendChild(permissionEl);
                }
            });
...
export async function saveUserChanges() {
    ...
    const permissionCheckboxes = document.querySelectorAll('.pod-permission-checkbox');
    permissionCheckboxes.forEach(checkbox => {
        const uid = checkbox.dataset.uid;
        const canCreate = checkbox.checked;
        const docRef = doc(db, 'users', uid);
        batch.update(docRef, { can_create_pod: canCreate });
    });


    try {
        await batch.commit();
        showNotification("User details updated successfully!", false);
...
export async function handleSkipFeedback() {
    if (!activeDeliveryForCompletion) {
        showNotification("Could not find the delivery to skip feedback for.", true);
        return;
    }
    showLoader();
    try {
        const deliveryRef = doc(db, 'deliveries', activeDeliveryForCompletion.id);
        await updateDoc(deliveryRef, { feedbackStatus: 'skipped' });
        showNotification("Feedback skipped for this delivery.", false);
        closeModal('delivery-completion-modal');
    } catch(error) {
        console.error("Error skipping feedback:", error);
        showNotification("Could not update feedback status.", true);
    } finally {
        hideLoader();
    }
}
...
// --- Live Location ---
const truckIconSvg = `<svg viewBox="0 0 640 512" xmlns="http://www.w3.org/2000/svg" fill="#0E639C" stroke="#fff" stroke-width="20"><path d="M544 0C597 0 640 43 640 96V352c0 35.3-28.7 64-64 64H512V320c0-17.7-14.3-32-32-32H160c-17.7 0-32 14.3-32 32v96H64c-35.3 0-64-28.7-64-64V96C0 43 43 0 96 0h448zM128 96c-17.7 0-32 14.3-32 32s14.3 32 32 32 32-14.3 32-32-14.3-32-32-32zm448 32c0-17.7-14.3-32-32-32s-32 14.3-32 32 14.3 32 32 32 32-14.3 32-32zM96 256c-17.7 0-32 14.3-32 32s14.3 32 32 32 32-14.3 32-32-14.3-32-32-32zm384-32c17.7 0 32-14.3 32-32s-14.3-32-32-32-32 14.3-32 32 14.3 32 32 32zM512 448c0 17.7-14.3 32-32 32H160c-17.7 0-32-14.3-32-32s14.3-32 32-32h320c17.7 0 32 14.3 32 32z"/></svg>`;
...
export async function handleStartDelivery(deliveryId) {
    ...
    const watchId = navigator.geolocation.watchPosition(
        ...
        },
        {
            ...
            maximumAge: 5000, // Request a fresher position
            timeout: 10000, 
        }
    );
...
function openMapModal(deliveryData) {
    openModal('map-modal');
    ...
    setTimeout(() => {
        ...
        mapInstance = L.map('map-container').setView([deliveryData.liveCoordinates.lat, deliveryData.liveCoordinates.lng], 15);
        ...
        truckMarker = L.marker([deliveryData.liveCoordinates.lat, deliveryData.liveCoordinates.lng], {
            icon: truckIcon
        }).addTo(mapInstance)
          .bindPopup(`<b>Driver:</b> ${deliveryData.driverName}<br><b>Job:</b> ${deliveryData.jobFileData.jfn}`)
          .openPopup();
        
        // Manual Refresh Button
        const refreshBtn = document.getElementById('refresh-map-btn');
        const refreshHandler = () => {
             if(mapInstance && truckMarker) {
                mapInstance.panTo(truckMarker.getLatLng());
            }
        };
        ...
        trackingUnsubscribe = onSnapshot(deliveryRef, (doc) => {
            if (doc.exists()) {
                const updatedDelivery = doc.data();
                if (updatedDelivery.liveCoordinates) {
                    const newLatLng = [updatedDelivery.liveCoordinates.lat, updatedDelivery.liveCoordinates.lng];
                    if (truckMarker) {
                        truckMarker.setLatLng(newLatLng);
                    }
                    if(mapInstance) {
                        mapInstance.panTo(newLatLng); // Automatically pan to new location
                    }
                }
                 if(updatedDelivery.status !== 'In Progress') {
                    closeMapModal();
                }
            }
        });
