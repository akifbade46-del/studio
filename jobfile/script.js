...
async function loadChargeDescriptions() {
    ...
    if (stored) {
        chargeDescriptions = JSON.parse(stored);
    } else {
         chargeDescriptions = [
...
async function saveJobFile() {
    if (!db) { window.showNotification("Database not connected.", true); return; }
    ...
    for (const check of checks) {
        try {
                ...
                if (isUpdating) {
                    for (const foundDoc of querySnapshot.docs) {
                        if (foundDoc.id !== docId) {
                            window.hideLoader();
                            window.showNotification(`Duplicate ${check.label} "${check.value}" found in job file: ${foundDoc.data().jfn}`, true);
                            return;
                        }
                    }
                } else {
                    window.hideLoader();
                    window.showNotification(`Duplicate ${check.label} "${check.value}" already exists in job file: ${querySnapshot.docs[0].data().jfn}`, true);
                    return;
                }
            }
        } catch (error) { 
            ...
            window.showNotification("Could not verify uniqueness. Please try again.", true);
            return;
        }
    }
    ...
    try {
        ...
        window.showNotification("Error saving job file.", true);
    }
}

async function checkJobFile(docId = null) {
    if (!currentUser || !['admin', 'checker'].includes(currentUser.role)) {
        window.showNotification("You do not have permission to check files.", true);
        return;
    }
    ...
    try {
        ...
        window.showNotification("Error checking job file.", true);
    }
}

async function uncheckJobFile(docId) {
    if (!currentUser || !['admin', 'checker'].includes(currentUser.role)) {
        window.showNotification("You do not have permission to uncheck files.", true);
        return;
    }
    window.showLoader();
    const uncheckData = {
    ...
    try {
        ...
        window.showNotification("Error unchecking job file.", true);
    }
}

async function approveJobFile(docId = null) {
    if (currentUser.role !== 'admin') {
        window.showNotification("Only admins can approve job files.", true);
        return;
    }
    ...
    try {
        ...
        window.showNotification("Error approving job file.", true);
    }
}
...
async function rejectJobFile() {
    ...
    try {
        ...
        window.showNotification("Error rejecting job file.", true);
    }
}
...
function loadJobFiles() {
    ...
    onSnapshot(q, (querySnapshot) => {
        ...
        window.showNotification("Error loading job files.", true);
    });
}
...
async function loadJobFileById(docId) {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            window.showNotification("Document not found.", true);
        }
        window.hideLoader();
    } catch (error) {
        window.hideLoader();
        console.error("Error loading document:", error);
        window.showNotification("Error loading job file.", true);
    }
}
...
async function previewJobFileById(docId) {
    ...
    try {
        ...
        window.showNotification("Error previewing job file.", true);
    }
}
...
async function moveToRecycleBin(docId) {
    ...
    try {
        ...
        window.showNotification("Error deleting job file.", true);
    } finally {
        window.hideLoader();
    }
}
...
async function openAdminPanel() {
    if (currentUser.role !== 'admin') {
        window.showNotification("Access denied.", true);
        return;
    }
    window.showLoader();
    ...
    window.hideLoader();
    window.openModal('admin-panel-modal');
}
...
async function saveUserChanges() {
    ...
    try {
        await batch.commit();
        window.hideLoader();
        window.showNotification("User details updated successfully!");
        window.closeModal('admin-panel-modal');
    } catch (error) {
        window.hideLoader();
        console.error("Error updating roles: ", error);
        window.showNotification("Failed to update user details.", true);
    }
}

async function backupAllData() {
    if (currentUser.role !== 'admin') {
        window.showNotification("Access denied. Only admins can perform backups.", true);
        return;
    }
    window.showLoader();
    try {
        ...
        window.showNotification("Backup created and download started successfully.");

    } catch (error) {
        console.error("Backup failed:", error);
        window.showNotification("An error occurred during backup.", true);
    } finally {
        window.hideLoader();
    }
}
...
async function handleRestoreFile(event) {
    ...
    if (currentUser.role !== 'admin') {
        window.showNotification("Access denied. Only admins can restore data.", true);
        return;
    }
    ...
    reader.onload = (e) => {
        ...
        try {
            ...
            if (!backupData.data || !backupData.data.jobfiles || !backupData.data.users) {
                window.showNotification("Invalid backup file format.", true);
                return;
            }
            ...
            const onOk = async () => {
                ...
                try {
                    ...
                    window.showNotification("Data restored successfully! The page will now reload.");
                    setTimeout(() => window.location.reload(), 2000);

                } catch (error) {
                    console.error("Restore failed:", error);
                    window.showNotification("An error occurred during restore. Data may be partially restored.", true);
                } finally {
                    window.hideLoader();
                }
            };
            ...
            window.showNotification("Failed to read or parse the backup file. Please ensure it's a valid JSON backup.", true);
        } finally {
            event.target.value = '';
        }
    };
        ...
        <div class="border border-gray-700 p-2 bg-white">
            <div class="grid grid-cols-12 gap-px bg-gray-700" style="border: 1px solid #374151;">
                <div class="col-span-3 bg-white p-1 flex items-center" style="border: 1px solid #374151;">
                     <img src="https://qgocargo.com/logo.png" alt="Q'go Cargo Logo" class="h-16 w-auto">
                </div>
...
function printPage() {
    const data = getFormData();
    // Calculate totals just before printing
    data.totalCost = parseFloat(document.getElementById('total-cost').textContent) || 0;
    data.totalSelling = parseFloat(document.getElementById('total-selling').textContent) || 0;
    data.totalProfit = parseFloat(document.getElementById('total-profit').textContent) || 0;
...
async function generateRemarks() {
    ...
    try {
        ...
        if (result.candidates && result.candidates[0]?.content?.parts?.[0]) {
            ...
            window.showNotification("Remarks generated successfully! ✨");
        } else {
        ...
        window.showNotification("Could not generate remarks. Please try again.", true);
    } finally {
        window.hideLoader();
    }
}
...
async function suggestCharges() {
    ...
    try {
        ...
        const prompt = `
            ...
            Which of the predefined charges are most relevant and what are their estimated costs and selling prices? Return the result as a JSON object with a single key "charges" which is an array of objects, each with "chargeName", "cost", and "selling" properties.
        `;
...
function openClientManager() { window.openModal('client-manager-modal'); }
...
function loadClients() {
    ...
    onSnapshot(query(clientsCollection), (snapshot) => {
        ...
        window.showNotification("Could not load clients.", true);
    });
}
...
async function saveClient(event) {
    ...
    if (!clientName) {
        window.showNotification("Client name is required.", true);
        return;
    }
    ...
    try {
        ...
        window.showNotification("Could not save client.", true);
    } finally {
        window.hideLoader();
    }
}
...
async function deleteClient(clientId) {
    ...
    try {
        ...
        window.showNotification("Client deleted successfully.");
    } catch (error) {
        console.error("Error deleting client:", error);
        window.showNotification("Could not delete client.", true);
    } finally {
        window.hideLoader();
    }
}
...
function displayJobsInModal(jobs, title) {
    ...
    window.openModal('user-jobs-modal', true);
}
...
async function openRecycleBin() {
    ...
    try {
        ...
        window.openModal('recycle-bin-modal', true);
    } catch (error) {
        console.error("Error opening recycle bin:", error);
        window.showNotification("Could not open recycle bin.", true);
    } finally {
        window.hideLoader();
    }
}
...
async function restoreJobFile(docId) {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            window.showNotification("Job file restored successfully.");
        ...
        window.showNotification("Error restoring file.", true);
    } finally {
        window.hideLoader();
    }
}
...
async function permanentlyDeleteJobFile(docId) {
    ...
    try {
        const deletedDocRef = doc(db, 'deleted_jobfiles', docId);
        await deleteDoc(deletedDocRef);
        window.showNotification("Job file permanently deleted.");
        openRecycleBin();
    } catch (error) {
        console.error("Error permanently deleting file:", error);
        window.showNotification("Could not permanently delete file.", true);
    } finally {
        window.hideLoader();
    }
}
...
function openChargeManager() {
    ...
    window.openModal('charge-manager-modal');
}
...
// Expose functions to the global scope to be accessible by HTML onclick attributes
