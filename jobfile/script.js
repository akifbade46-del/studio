...
function initializeFirebase() {
    try {
        ...
        document.body.innerHTML = '<h1>Error: Could not connect to the database.</h1>';
    }
}
...
function setupChargeAutocomplete(inputElement) {
    ...
    inputElement.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'ArrowDown') { e.preventDefault(); activeSuggestionIndex = (activeSuggestionIndex + 1) % suggestions.length; }
        else if (e.key === 'ArrowUp') { e.preventDefault(); activeSuggestionIndex = (activeSuggestionIndex - 1 + suggestions.length) % suggestions.length; }
        ...
        suggestions.forEach((s, i) => s.classList.toggle('selected', i === activeSuggestionIndex));
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('client-search-bar').addEventListener('input', (e) => {
        const filtered = clientsCache.filter(c => c.name.toLowerCase().includes(e.target.value.toLowerCase()));
    ...
    // Make functions globally available for inline event handlers in dynamic HTML
